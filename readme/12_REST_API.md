...
| المسار | يسكن |
|---|---|
| مسارُ **شريحةٍ بالتطبيق** | `core/infra/network/rest/api_urls.dart` |
| مسارُ **موديولٍ اختياريّ** | `lib/modules/<name>/data/<name>_urls.dart` — **بجواره** |

**والقاعدة أن `ApiUrls` يجيب على سؤالٍ واحد: «ما الذي يطلبه هذا التطبيق؟»**
وخلطُ مسارات موديولٍ **قد يكون مطفأً** به يجعل الجوابَ يحمل ما لا يُنادى — ومن
يقرأ الملف ليعرف ما يخدمه الباك يجد أجوبةً صحيحة وسط مقترحاتٍ موسومة بتعليق،
**والوسمُ يُقرأ حين يُبحث عنه لا حين يُتصفَّح**.

فمن يُشعل موديولاً يجد مساراتِه بجواره، ومن يُطفئه لا يراها أصلاً:
`AccessControlUrls` · `DataTransferUrls` · `MultiDeviceUrls`.
...
| `ui/error/failure_ui_mapper.dart` | Failure → UiAction |
