# Realtime Push (`modules/realtime/`) — وثيقة تصميم لموديول **لم يُبنَ**

> **الحالة:** مرحلة التحليل والتصميم — **لم يُبنَ بعد، ولا يوجد كود له بالقالب.**
> **الهدف:** الخادم يدفع التغيير للأجهزة المتصلة بنفس الحساب فور وقوعه.

### ⚠️ ثلاثة أشياء متجاورة تُخلط — والعنوان القديم كان يخلطها

كان اسم هذا الملف «Multi-Device Session Sync»، و`modules/multi_device/` **مبنيّ**
بالقالب (مطفأ بعَلَمه — لا «غير موجود»). فكان سطر «لم يُبنَ بعد» يُقرأ عن الموديول
المجاور، وهو أدقّ ما يمكن أن يخطئ فيه عنوان.

| | السؤال الذي يجيبه | حالته بالقالب |
|---|---|---|
| `modules/multi_device/` | «أين أنا مسجَّل الدخول الآن، وكيف أُنهي جلسة؟» | **مبنيّ** — مطفأ بـ`AppFeatures.multiDevice = false` |
| `modules/sync/` ([`sync.md`](sync_design_spec.md)) | «ماذا فاتني وأنا أوفلاين؟» — الكلاينت يسحب | **مبنيّ** — مطفأ بـ`AppFeatures.offlineSync = false` |
| **هذه الوثيقة** | «ماذا تغيّر على جهاز آخر **الآن**؟» — الخادم يدفع | **غير موجود** — لا مجلد ولا علم ولا سطر |

> 📍 **لماذا هذا الملف هنا لا بـ`lib/modules/realtime/`؟**
> كان يسكن هناك — مجلد باسم موديول لا يحوي إلا هذه الوثيقة، بلا سطر Dart واحد.
> فكان يظهر بكل قائمة مجلدات وبشجرة `10_ARCHITECTURE.md` وبكل أداة تمشي على `lib/`
> كأنه موديول قائم. **مجلد فارغ باسم ميزة هو ادّعاء بوجودها**، ومن يقرأ الشجرة
> يعدّها مبنيّة ولا يفتحها ليكتشف. التصميم قائم وله قيمة — مكانه `readme/` مع
> بقية ما لم يُبنَ بعد، حتى يصير كوداً.
