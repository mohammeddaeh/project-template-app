...
> 📎 **[`integration_audit.md`](integration_audit.md)** — تدقيق عقد الـwire بين `app_template` و`backend_template` (2026-08-11) وسجلّ إصلاحه (2026-08-12).
> خمسة أعطال قاطعة كانت تمنع مسار الدخول كلياً — **كلها مُصلَحة ومُثبَّتة باختبارات الطرفين**. اقرأها قبل أي عمل يمسّ المصادقة أو شكل أي رد.
...
| **05** | Crash Reporting (Sentry/Crashlytics) | 🟡 متوسطة | ⬜ | **✅** | `modules/crash_reporting/` + `AppFeatures.crashReporting` |
| 06 | Screenshot Prevention | 🟡 متوسطة | ⬜ | ⬜ | Platform Service + feature flag |
| 07 | Deep Linking | 🟡 متوسطة | ⬜ | ⬜ | Router pattern |
| **08** | App Lifecycle Service | 🟡 متوسطة | ⬜ | **✅** | `core/platform/lifecycle/` + `AppFeatures.appLifecycle` |
...
| 19 | `MutationResult` — نتيجة الطفرة تُرجَع ولا تُبَثّ | 🔴 عالية | ✅ | ◐ | `presentation/shared/` — **الملف وصل، ولا مستهلك له هنا** |
| **20** | `message_key` — سبب الرفض يُقرأ آلياً لا نصّياً | 🔴 عالية | ✅ | **✅** | **النصفان معاً منذ 2026-08-17**: الخادم يرسل + `BusinessFailure.messageKey` يرفعه بالعميل |
| **21** | `queryBooleanSchema` — البوليان بالـquery لا يُكرَه | 🔴 عالية | ✅ | **✅** | Zod schema مشترك — **`backend_template`** |
| **22** | Contract Smoke Test — يؤكد العلاقات لا الأرقام | 🔴 عالية | ✅ | **✅** | `npm run smoke` + ١٧٣ فحص وحدة — **`backend_template`** |
...
| 30 | طبقة الترجمة البعيدة: كاش عند الإقلاع + تحرير داخل التطبيق | 🟡 متوسطة | ✅ | ⛔ | **محجوب بالباك** — `backend_template` بلا `languages`/`translation_entries` |
...
>
> `⛔` = **محجوب بشيء خارج هذا الملف** — مُسمّى صراحةً، فلا يُقرأ كعملٍ متاح الآن.
>
> `◐` = **جزئي، والناقص مُسمّى** — لا «شبه منتهٍ»:
> - **#19** الملف `presentation/shared/mutation_result.dart` موجود، و**لا يستورده شيء بالقالب**. البند وُلد من عطلين حقيقيين بقرطاس (`RoleDetailState.saveFailed` كحالة · `TranslationsState.saveFailed` كعلم لزج)، والقالب ليس فيه بعدُ طفرةٌ تُعلن نتيجتها فتحتاجه. **`✅` كانت تعني «الملف وصل» وتُقرأ «النمط يعمل هنا»** — والفرق هو بالضبط الفرق بين سطح قالبٍ وسلسلةٍ عاملة. يُغلق حين يستهلكه أول cubit طفرة بالقالب، لا قبله.
> - **#25** `DetailHeaderCard`/`DetailFact`/`DetailSectionHeader` وصلت **مع انعكاس المرآة**؛ **`PermissionsPanel` لا**. و**الناقص ليس نسخاً من قرطاس**: النسختان تحلّان تسمية الصلاحية بطريقتين مختلفتين — القالب يقرأ `LocalizedLabel {ar, en}` **من كتالوج الخادم** (`permission_catalog.dart`)، وقرطاس يقرأ من `DynamicLocalizationService` (موديول غير موجود هنا — البند #30). فنسخ ملف قرطاس هنا يجلب معه تبعيةً محجوبة. الباقي فعلاً: استخراج الرسم من `permission_matrix.dart` إلى `shared/widgets/layout/` ليستعمله العارض والمحرّر معاً — **بمصدر تسمية القالب، لا بمصدر قرطاس**.
...
>
> **أُغلقت لاحقاً** (2026-08-17، أثناء مراجعة القالب الشاملة): **#05** و**#08** كانا `⬜` بينما `modules/crash_reporting/` و`core/platform/lifecycle/` مبنيان بعَلَمَيهما منذ مدة؛ **#22** كان `◐` بحجّة «القالب بلا `features/`» وفيه الآن `auth` و`account` و`notes` بـ١٧٣ فحصاً؛ **#31** كان `◐` بحجّة «يستورد من موديول غير موجود» وفيه الآن `core/authz/middleware/require-permission.ts` كاملاً بفحص تغطية المسارات.
>
> الأربعة تشترك بسبب واحد: **الجدول لم يُعَد قراءته بعد أن نُفِّذت البنود التي كان ينتظرها.** هذه الوثيقة تصف الحاضر، فصفٌّ متأخّر عن الواقع يقرأه القارئ عملاً باقياً ويبنيه مرة ثانية.
>
> **وفُتحت ثلاثة** (2026-08-17، عند مقارنة القالب بقرطاس صفّاً صفّاً): **#19** من `✅` إلى `◐` — الملف وصل بلا مستهلك؛ **#25** بقي `◐` لكن **سببه المكتوب كان خاطئاً** (وصف الناقص بأنه استخراج، وهو في الحقيقة اختلاف مصدر التسمية)؛ **#30** من `⬜` إلى `⛔` — «الموديول غير موجود بالقالب» وصفٌ للعَرَض، والسبب أن `backend_template` بلا endpoints ترجمة أصلاً، فبناؤه بالفرونت وحده يُنتج موديولاً يكلّم العدم.
>
> **والخطأ الثالث هو الأخطر شكلاً**: صفٌّ يذكر عائقاً **قابلاً للحل** بينما العائق الحقيقي بمستودع آخر، يجعل القارئ يبدأ عملاً لا يمكن أن ينتهي.
>
> **القاعدة المستخلَصة**: `✅` بعمود «القالب» تعني **«له مستهلك هنا»**، لا «الملف موجود». الفرق بينهما هو نفسه الفرق الذي تجمعه هذه الوثيقة كلها — وأن تقع فيه الوثيقة عن نفسها هو أسوأ شكل يقع به.
>
> ### وجولة المزامنة الثنائية (2026-08-17) كشفت نصفاً ناقصاً ثالثاً
>
> **#20 كان `✅✅` والنصف العميل غير موجود.** `backend_template` يرسل `message_key` و`check:messages` يفرضه — لكن `BusinessFailure` هنا **لم يكن يحمل الحقل أصلاً**، فلا شيء يرفعه عن السلك. فكل رفض ينهار إلى اللوحة الحمراء نفسها، ولا يستطيع أي مستدعٍ تمييز قاعدة عن قاعدة.
>
> وانكشف بالطريقة الوحيدة التي كان يمكن أن ينكشف بها: نُقل `business_failure_message_key_test.dart` من قرطاس، **فرفض أن يُصرَّف**. لا `dart analyze` ولا أي اختبار قائم كان ليقوله — الحقل الغائب ليس خطأ، هو ببساطة غياب.
>
> **ومعه عطل ثانٍ بنفس الملف**: كل `409` كان يُخرَّج إلى `ConflictFailure` ⇒ `Silent`، بما فيها رفض قواعد العمل. فالمستخدم يضغط ولا يحدث شيء ولا سبب يُذكر — والخادم أرسل السبب. صار التمييز بشكل الجسم (`server_version`/`client_version`/`conflict_fields`)، ولا endpoint يُصدر شكل المزامنة أصلاً.
>
> **الدرس**: نقل اختبارٍ من مشروع مشتقّ هو أرخص فاحص انحراف موجود. الاختبار الذي لا يُصرَّف يسمّي الفجوة بالسطر والحرف.
>
...
```
التفاصيل الكاملة خطوة بخطوة (مع أمثلة كود كاملة وchecklist): `qirtas_backend/docs/12_REST_API.md` §6.1 (عقد الـREST) و`qirtas_app/readme/14_PAGINATION.md` §5.1 (التنفيذ بالفرونت).
...
```
التفاصيل الكاملة بالأمثلة: `readme/20_WIDGETS.md`. ومرآة تفاعلية بـ`WidgetLibraryDemoScreen` (قانون المرآة).
...
القالب كان يوفّر طريقين للأنماط: `context.textTheme.headlineLarge` (Material) و`context.ts.headlineLg` (`AppTextStyles`) — ويبدوان بديلين متكافئين، **وليسا كذلك**. طُبِّقت الميزة بـ`_textTheme()` وحده لأنه المصدر الذي يبني `AppTextStyles` عليه بـ`copyWith`، فوصلت الطريقين معاً؛ ولو طُبِّقت بالعكس لظهر **العنوان نفسه مختلفاً بشاشتين** حسب أي وصول استعملته كل واحدة — فرقٌ لا يُنسَب لسببه أبداً.

> **وحُسم الأمر لاحقاً (2026-08-17): `app_text_styles.dart` حُذف.** لم يكن له مستدعٍ واحد بـ`lib/`، وكان يناقض قاعدة الطباعة الصارمة بـ`lib/CLAUDE.md` (`context.textTheme` حصراً). القاعدة أدناه تبقى — لكن مصدر الأنماط صار **واحداً** بدل أن يُدار اثنان بانضباط.
