...
```dart
// CORRECT — template convention (from core-10_ARCHITECTURE.mdc):
result.fold(
  (failure) {
...
**Failure handling follows template convention (core-10_ARCHITECTURE.mdc):**
...
**Why:** `Cubit` is from `flutter_bloc`, a Flutter dependency. Template rule (`.cursor/rules/core-10_ARCHITECTURE.mdc`):
```
