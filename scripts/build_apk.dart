...
///
/// ⚠️ **ولا قارئ له اليوم.** كان الرأس يقول إن `scripts/install.dart` يقرأه
/// لمعرفة مسار APK ورقم الحزمة — وذلك السكربت **غير موجود بالمستودع**، ولا شيء
/// آخر يفتح هذا الملف. صُحِّح 2026-08-19.
///
/// يُترك مكتوباً لأنه رخيص ويصف آخر بناء بدقّة، فمن أراد سكربت تثبيت (`adb
/// install`) أو خطوة CI ترفع الأثر يجد ما يحتاجه جاهزاً. لكن لا تعتمد على وجود
/// مستهلك له قبل أن تفتح المجلد وتتأكّد — وهي القاعدة نفسها بـ`readme/03_RULES.md` §R28.
library;
...
Future<void> main(List<String> args) async {
  ...
  if (!noCodegen) {
    ...
    await _stream('dart', ['run', 'scripts/gen_code.dart']);
  }
