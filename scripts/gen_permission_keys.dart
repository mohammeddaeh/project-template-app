...
/// cd ../app_template && dart run scripts/gen_permission_keys.dart --update --from ../perms.json
/// #   أو من خادم يعمل:
/// dart run scripts/gen_permission_keys.dart --update --url http://localhost:3000/api/v1 --token <t>
///
/// # التحقّق (للـCI): يفشل إن كان القفل متقادماً
/// dart run scripts/gen_permission_keys.dart --verify --from ../perms.json
/// ```
///
/// ── المشكلة ────────────────────────────────────────────────────────────────
///
...
void _writeLock(List<String> keys) {
  final lock = {
    '_comment': 'GENERATED — do not edit by hand. '
        'Run: dart run scripts/gen_permission_keys.dart --update --from <perms.json>',
...
void _verify(List<String> incoming) {
  ...
  stderr.writeln(
    '\n   شغّل: dart run scripts/gen_permission_keys.dart --update --from <perms.json>',
  );
  exitCode = 1;
}
...
String _render(List<String> keys) {
  final buffer = StringBuffer()
    ...
    ..writeln('// dart run scripts/gen_permission_keys.dart')
    ..writeln('//')
