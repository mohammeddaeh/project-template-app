...
> **مرجعٌ حيٌّ واحد مبنيٌّ بالكامل:** `auth/` — دورة الحساب.
> انسخ بنيتَه حرفياً؛ والقواعد أدناه مستخرَجة منه لا مكتوبة قبله.
>
> ⛔ **و`notes/` حُذفت (2026-09-08)** — كانت الـfeature المرجعية لـCRUD. فأنماط
> §CRUD-PATTERNS أدناه **موصوفةٌ ولا شريحةَ تعرضها**، وأولُ قائمةٍ تبنيها هي أولُ
> تطبيقٍ لها. و`PaginationCubit` كذلك: مبنيٌّ ومكتمل وبلا مستهلكٍ واحد.
...
import 'package:app_template/ui/feedback/feedback_extension.dart'; // ⚠️ منفصل
```
...
> **Import الوحيد:** `package:app_template/ui/widgets/widgets.dart`
...
## Shimmer — الهيكلُ هو الشاشةُ نفسُها

**⛔ لا ملفَّ `*_skeleton.dart`.** يُلفّ الجسدُ الحقيقيّ، ويُلبَس كلُّ حقلٍ
ديناميكيّ عظماً بمقاس ما سيحلّ محلَّه:

```dart
// الشاشة
loading: () => MyBody.loading(),        // نفسُ الودجة ببياناتٍ نائبة
loaded: (data) => MyBody(data: data),

// الجسد
Skeletonized(enabled: loading, child: Column(children: [...]))

// الودجة — البيانة ShimmerText، واللافتة Text
ShimmerText(item.title, style: context.textTheme.titleSmall)
Text(LocaleKeys.itemTitle.tr())
Bone.circle(child: AvatarWidget(name: n))   // للأشكال — و`AvatarWidget`
                                            // و`StatusChip` و`AppListTile`
                                            // تعرفها بأنفسها
```

القواعد الأربع: [`readme/21_WIDGETS_USAGE.md`](../../readme/21_WIDGETS_USAGE.md) §18.

> ⚠️ **وعائلةٌ أقدمُ ما زالت بالمكتبة** — `ShimmerLoadingWidget` و`SkeletonWidget`
> و`skeleton_templates.dart` — يستعملها `PageLoadingIndicator` وأخواتُه من الداخل.
> **لا تبنِ بها حالةَ تحميلٍ لشاشة**: مقاساتُها تُكتب عدداً (`width: 120, height: 14`)
> فتُقارب الجسدَ ولا تطابقه، وتتقادم مع كلِّ تعديلٍ عليه. القاعدةُ أعلاه هي المسار.
...
| المكان | متى |
|---|---|
| `lib/ui/widgets/` | يُستخدم في أكثر من Feature |
| `lib/features/{f}/presentation/widgets/` | خاص بـ Feature واحدة |

بعد إضافة widget مشترك → أضفه في `lib/ui/widgets/widgets.dart`:
```dart
...
- [ ] تحقق من `lib/ui/widgets/` أولاً — لا تُعيد بناء موجود
