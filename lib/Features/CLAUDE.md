...
| الدرس | أين |
|---|---|
| **الرفض ليس حالة واحدة** | `login_repository_impl.dart` يفرّق أربع حالات 403 عبر `ApiError.data['account_status']` — **لا بمطابقة نصّ الرسالة**، لأنها نثرٌ مترجَم يتغيّر بالصياغة |
| نجاحٌ لا يعني «ادخل» | `pendingApproval`/`rejected` نجاحٌ باعتماد الهوية وطريقٌ مسدود بالحساب — `login_screen.dart` يوثّق التفرّع |
| التوكن بمكان واحد | `SessionRepository` بـ`core/infra/session/` — والشريحة تستدعيه، لا تُنشئ نسخة |
| المستخدم الحالي يُخزَّن | `CurrentUserRepository` يحفظ لقطة غير حسّاسة، فلا تبدأ الجلسة بصلاحيات فارغة بعد كل إقلاع |

**ما لم يُشحن عمداً**: `register/` وحدها — لأن شكلها يختلف جذرياً بين مشروع وآخر (بموافقة أو بدونها، بطلب دور أو بدونه). `ApiUrls.register` جاهز، وبناؤها نسخٌ من `login/`.

**والعقد المطلوب من الباك** لشريحتَي كلمة المرور:

| Endpoint | موثَّق؟ | الجسم | الشرط الحاسم |
|---|---|---|---|
| `POST /forgot-password` | لا | `{email}` | **نفس الجواب** للمسجَّل وغير المسجَّل — وإلا فهو عرَّاف عضوية |
| `POST /reset-password` | لا | `{email, token, new_password}` | الرمز **يُستهلك مرة واحدة** وله انتهاء |
| `POST /change-password` | نعم | `{current_password, new_password}` | كلمة حالية خاطئة → **422** لا 401 |

**والملف الوحيد الذي يُتوقَّع تعديله فوراً**: `auth/shared/entities/auth_user.dart` — يطابق شكل مستخدم **الـAPI الخاص بك**، وهو موثَّق بأنه غير مُزامَن مع أي مشروع.
