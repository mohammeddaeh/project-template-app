...
import 'package:app_template/ui/theme/app_palette.dart';
...
class AppColors extends ThemeExtension<AppColors> {
  ...
  /// أرضيةُ عنصرٍ حاضرٍ **غيرِ متاح**.
  ///
  /// **سطحٌ حقيقيّ، لا اللونُ المتاحُ بشفافية.** تخفيفُ `primary` إلى ٤٠٪ فوق
  /// أبيضَ ينتج `#A8BFD6`، والأبيضُ عليه يقيس **١٫٩١:١** — لافتةُ زرٍّ معطَّل
  /// **غيرُ مقروءة**، بلوحة القالب نفسِها، وبأوّل شاشة.
  ///
  /// والشفافيةُ تبدو **حالةً معطَّلة** لمن يكتبها و**عطلَ رسمٍ** لمن يقرأها —
  /// ولا يمسكها `dart analyze` ولا اختبار: كلاهما لونٌ صالح.
  final Color bgDisabled;

  /// لافتةٌ فوق [bgDisabled] — **٦٫٥٩:١** بالوضعين. خافتةٌ وتُقرأ.
  final Color textDisabled;

  /// المحتوى فوق **سطح العلامة** — الشاشة المطلية بلون الهوية (دخول · سبلاش ·
  /// ترويسة ملوّنة). و`primary` **لا ينوب عنه**: ذاك ينقلب مع الوضع، وسطحُ
  /// العلامة لا ينقلب — فيصير النصُّ فوقه غيرَ مقروء بأحد الوضعين.
  ///
  /// > ⬜ **وبقيّةُ عائلة سطح العلامة** (`bgBrand` · `bgBrandDeep`) لم تُضَف بعد:
  /// > لا مستهلكَ لها بالقالب اليوم، و`✅` تعني «له مستهلك هنا» لا «الملف وصل»
  /// > (R27). تُضاف يوم تُبنى أوّلُ شاشةٍ مطليّة بالهوية.
  final Color onBrand;

  /// اللمسةُ المميِّزة فوق سطح العلامة — تقدُّمٌ, أو قيمةٌ تُبرَز.
  ...
  factory AppColors.light() {
    return AppColors(
      ...
      bgDisabled: AppPalette.gray300,
      textDisabled: AppPalette.gray700, // 6.59:1 over bgDisabled
      onBrand: AppPalette.white,
      onBrandAccent: AppPalette.secondaryMain,
      ...
      statusSuccessFg: AppPalette.successDarker, // 6.79:1 over success16
      statusWarningBg: AppPalette.warning16,
      statusWarningFg: AppPalette.warningDarker, // 6.34:1 over warning16
  ...
  factory AppColors.dark() {
    return AppColors(
      ...
      bgDisabled: AppPalette.gray700,
      textDisabled: AppPalette.gray300, // 6.59:1 over bgDisabled
      onBrand: AppPalette.white,
      onBrandAccent: AppPalette.secondaryMain,
      ...
      statusSuccessFg: AppPalette.successDarker, // 6.11:1 over success32
      statusWarningBg: AppPalette.warning32,
      statusWarningFg: AppPalette.warningDarker, // 4.87:1 over warning32
      statusErrorBg: AppPalette.error32,
      statusErrorFg: AppPalette.errorDarker, // 6.13:1 over error32
      statusInfoBg: AppPalette.info32,
      statusInfoFg: AppPalette.infoDarker, // 7.82:1 over info32
